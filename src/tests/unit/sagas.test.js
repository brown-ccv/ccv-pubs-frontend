...
describe('make getData request', () => {
  ...
  it('should call api and dispatch CHANGE_ERROR action', async () => {
    const getData = jest.spyOn(Client, 'getData').mockImplementation(() => Promise.reject());
    const dispatched = [];
    const result = await runSaga({ dispatch: (action) => dispatched.push(action) }, fetchData);
...
describe('make getNgrams request', () => {
  it('should call api and dispatch CHANGE_NGRAMS action', async () => {
    ...
    const dispatched = [];
    const result = await runSaga({ dispatch: (action) => dispatched.push(action) }, fetchNgrams);
  ...
  it('should call api and dispatch CHANGE_ERROR action', async () => {
    const getNgrams = jest.spyOn(Client, 'getNgrams').mockImplementation(() => Promise.reject());
    const dispatched = [];
    const result = await runSaga({ dispatch: (action) => dispatched.push(action) }, fetchNgrams);
...
describe('make getDoiInfo request', () => {
  it('should call api and dispatch CHANGE_DOI_INFO action', async () => {
    ...
    const dispatched = [];
    const result = await runSaga({ dispatch: (action) => dispatched.push(action) }, fetchDoiInfo, [
      'doi',
    ]);
  ...
  it('should call api and dispatch CHANGE_ERROR action', async () => {
    const getDoiInfo = jest.spyOn(Client, 'getDoiInfo').mockImplementation(() => Promise.reject());
    const dispatched = [];
    const result = await runSaga({ dispatch: (action) => dispatched.push(action) }, fetchDoiInfo, [
      'doi',
    ]);
...
describe('make postPub request', () => {
  it('should call api and dispatch CHANGE_DATA, CHANGE_NGRAMS, and CHANGE_ADD_SUCCESS action', async () => {
    const dummyPub = {
      ...
      newngrams: JSON.stringify([{ word: 'cat', count: 4, freq: 33, pubs: '[117]' }]),
    };
    const postPubDummy = jest
      .spyOn(Client, 'postPub')
      .mockImplementation(() => Promise.resolve(dummyPub));
    const dispatched = [];
    const result = await runSaga({ dispatch: (action) => dispatched.push(action) }, postPub, [
