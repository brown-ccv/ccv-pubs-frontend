...
describe('make getData request', () => {
    it('should call api and dispatch CHANGE_DATA action', async () => {
        ...
        const getData = jest.spyOn(Client, 'getData')
        .mockImplementation(() => Promise.resolve(dummyPublications));
        const dispatched = [];
        const result = await runSaga(
            {dispatch: (action) => dispatched.push(action)}, fetchData);
    ...
    expect(getData).toHaveBeenCalledTimes(1);
    expect(dispatched).toEqual([actions.changeError("Could not connect to server.  Please refresh the page in a few minutes and try again.")]);
    getData.mockClear();
    })
})
...
describe('make getNgrams request', () => {
    it('should call api and dispatch CHANGE_NGRAMS action', async () => {
        ...
        const getNgrams = jest.spyOn(Client, 'getNgrams')
        .mockImplementation(() => Promise.resolve(dummyNgrams));
        const dispatched = [];
        const result = await runSaga(
            {dispatch: (action) => dispatched.push(action)}, fetchNgrams);
...
describe('make getDoiInfo request', () => {
    it('should call api and dispatch CHANGE_DOI_INFO action', async () => {
        const dummyDoiInfo = {data:{title: "test", author: "test-author", year: "2020"}, status: "old"}
        const getDoiInfo= jest.spyOn(Client, 'getDoiInfo')
        .mockImplementation(() => Promise.resolve(dummyDoiInfo));
        const dispatched = [];
        const result = await runSaga(
            {dispatch: (action) => dispatched.push(action)}, fetchDoiInfo, ["doi"]);
...
describe('make postPub request', () => {
    it('should call api and dispatch CHANGE_DATA, CHANGE_NGRAMS, and CHANGE_ADD_SUCCESS action', async () => {
        const dummyPub = {newdata:JSON.stringify([{title: "test", author: "test-author"}]), newngrams:JSON.stringify([{word: "cat", count: 4, freq: 33, pubs:"[117]"}])}
        const postPubDummy = jest.spyOn(Client, 'postPub')
        .mockImplementation(() => Promise.resolve(dummyPub));
        const dispatched = [];
        const result = await runSaga(
            {dispatch: (action) => dispatched.push(action)}, postPub, [{data:{title: "test1", author: "test-author"}}]);
