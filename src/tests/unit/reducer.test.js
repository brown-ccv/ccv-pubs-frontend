...
describe('selectors', () => {
  it('should select data from state', () => {
    ...
    expect(selectors.getData(state)).toStrictEqual([{title: "test", author:"Maura"}]);
});
it('should select ngrams from state', () => {
  const state = fullState;
  expect(selectors.getNgrams(state)).toStrictEqual([{word: "cat", count: 4, freq: 33, pubs:"[117]"}]);
});
...
it('should select doiInfo from state', () => {
  const state = fullState;
  expect(selectors.getDoiInfo(state)).toStrictEqual({data:{title: "testing-1234", author: "me2"}, status:"old"});
});
it('should select keycloak from state', () => {
  const state = fullState;
  expect(selectors.getKeycloak(state)).toStrictEqual({keycloak: "keycloak", authenticated: true, iscis: true, profile: "abc123"})
});
