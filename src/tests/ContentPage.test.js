...
function shallowSetupFull(){
    const props = {
        ...
        fetchNgrams: fetchNgrams,
        fetchData: fetchData,
        publications: [{title: "test", author: "me", year: 2020, url: "www.test.com"}],
        error: null,
        loading: false,
...
it('renders ComponentPage with correct display when loaded', () => {
    ...
    expect(Table.props().publications).toEqual([ { title: 'test', author: 'me', year: 2020, url: 'www.test.com' } ])

    expect(wrapper.find("h1").first().text()).toBe("Publications")
    expect(wrapper.find("FontAwesomeIcon").exists()).toBe(true)
    expect(wrapper.find("h3").exists()).toBe(true)
    expect(wrapper.find("h3").text()).toBe(" What are these publications all about? ")
...
it('renders with errortext when an error occurs', () => {
    ...
    expect(wrapper.find("div.error-text").children().props().children).toEqual("something went wrong");
    expect(wrapper.find("h1").first().text()).toBe("Publications")
    expect(wrapper.find("FontAwesomeIcon").exists()).toBe(true)
    expect(wrapper.find("h3").exists()).toBe(false)
    expect(wrapper.find("div.PubsTable-CP").exists()).toBe(false)
